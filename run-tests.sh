#! /usr/bin/env bash
# adapted from Operating Systems: Three Easy Pieces project code

# disable color output when redirected to a file
if [[ -f /proc/$$/fd/1 ]]; then
    GREEN=""
    RED=""
    NONE=""
...
print_error_message () {
    ...
    if (( $verbose == 1 )); then
        echo "  EXPECTED:"
        if [[ -e $testdir/$testnum.$filetype.re ]]; then
            cat $testdir/$testnum.$filetype.re
        else
            cat $testdir/$testnum.$filetype
        fi
        echo "  RESULT:"
        cat tests-out/$testnum.$filetype
    fi
...
check_test () {
    ...
    if [ -e $testdir/$testnum.$filetype.re ]; then
        returnval=$(grep -Ef $testdir/$testnum.$filetype.re tests-out/$testnum.$filetype)
    else
